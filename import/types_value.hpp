#ifndef _TYPES_VALUE_HPP
#define _TYPES_VALUE_HPP

#include "std_input.hpp"
#include "type_name_func.hpp"

// #include<concepts> // c++20 or above do not need include this library
/*
    to compile is needed C++20 - lattest
...
template <template <typename, typename> class BinaryOpr, typename ArgType, typename Type,
          typename... Types>
struct st_find_type<BinaryOpr, ArgType, jf::type_list_t<Type, Types...>> {
...
template <template <typename, typename> class BinaryOpr, typename ArgType, typename Type>
struct st_find_type<BinaryOpr, ArgType, jf::type_list_t<Type>> {
    static constexpr bool value = st_find_type<BinaryOpr, ArgType, Type>::value;
...
struct st_select_first_type<Type, Types...> {
    using type = Type;
    using typelist = jf::type_list_t<Type>;
};
//////////////////

template <>
struct st_select_first_type<jf::type_list_t<>> {
    using type = NoType;
    using typelist = jf::type_list_t<>;
};

template <typename Type>
struct st_select_first_type<jf::type_list_t<Type>> {
    using type = Type;
    using typelist = jf::type_list_t<Type>;
};

template <typename Type, typename... Types>
struct st_select_first_type<jf::type_list_t<Type, Types...>> {
    using type = Type;
    using typelist = jf::type_list_t<Type>;
};
...
template <>
struct st_select_last_type<jf::type_list_t<>> {
    using type = jf::NoType;
    using typelist = jf::type_list_t<>;
};

template <typename Type>
struct st_select_last_type<jf::type_list_t<Type>> {
    using type = Type;
    using typelist = jf::type_list_t<type>;
};

template <typename Type, typename... Types>
struct st_select_last_type<jf::type_list_t<Type, Types...>> {
...
template <auto SelectIndex>
struct st_select_nth_type<SelectIndex, jf::type_list_t<>> {
    using type = jf::NoType;
    using typelist = jf::type_list_t<>;
};

template <typename Type, typename... Types>
struct st_select_nth_type<0, jf::type_list_t<Type, Types...>> {
    using type = Type;
    using typelist = jf::type_list_t<type>;
};

template <auto SelectIndex, typename Type, typename... Types>
struct st_select_nth_type<SelectIndex, jf::type_list_t<Type, Types...>> {
...
struct st_push_front_type<ArgType, jf::type_list_t<Types...>> {
    using type = jf::type_list_t<ArgType, Types...>;
};
...
template <typename ArgType, typename... Types>
struct st_push_back_type<ArgType, jf::type_list_t<Types...>> {
    using type = jf::type_list_t<Types..., ArgType>;
};
...
template <>
struct st_pop_front_type<jf::type_list_t<>> {
    using type = jf::type_list_t<>;
};

template <typename ArgType, typename... Types>
struct st_pop_front_type<jf::type_list_t<ArgType, Types...>> {
    using type = jf::type_list_t<Types...>;
};
...
struct st_pop_back_type<jf::type_list_t<LeftTypes...>, jf::type_list_t<>> {
    using type = jf::type_list_t<LeftTypes...>;
};

template <typename... LeftTypes, typename Type, typename... RightTypes>
struct st_pop_back_type<jf::type_list_t<LeftTypes...>, jf::type_list_t<Type, RightTypes...>> {
    using leftlist = push_back_type_t<Type, jf::type_list_t<LeftTypes...>>;
    using rightlist = jf::type_list_t<RightTypes...>;

    using type = std::conditional_t<sizeof...(RightTypes) == 0, jf::type_list_t<LeftTypes...>,
                                    typename st_pop_back_type<leftlist, rightlist>::type>;
};

template <typename ListType>
using pop_back_type_t = st_pop_back_type<jf::type_list_t<>, ListType>::type;
...
template <typename ArgType>
struct st_is_type_in_list<ArgType, jf::type_list_t<>> {
    static constexpr bool value = false;
};

template <typename ArgType, typename Type, typename... Types>
struct st_is_type_in_list<ArgType, jf::type_list_t<Type, Types...>> {
...
template <typename... Types>
struct st_unique_type<jf::type_list_t<Types...>> {
    using type = jf::type_list_t<Types...>;
};

template <typename... Types, typename Type>
struct st_unique_type<jf::type_list_t<Types...>, Type> {
    using list = jf::type_list_t<Types...>;
    using type =
        std::conditional_t<is_type_in_list_v<Type, list>, list, jf::push_back_type_t<Type, list>>;
};

template <typename... Types, typename Type, typename... RightTypes>
struct st_unique_type<jf::type_list_t<Types...>, Type, RightTypes...> {
    using list = st_unique_type<jf::type_list_t<Types...>, Type>::type;
    using type = st_unique_type<list, RightTypes...>::type;
};

template <typename... Types, typename... RightTypes>
struct st_unique_type<jf::type_list_t<Types...>, jf::type_list_t<RightTypes...>> {
    using type = st_unique_type<jf::type_list_t<Types...>, RightTypes...>::type;
};

template <typename... Types>
using unique_type_t = st_unique_type<jf::type_list_t<>, Types...>::type;

template <typename LeftList, typename RightList>
struct st_prepend_type;

template <typename... LeftTypes, typename... RightTypes>
struct st_prepend_type<jf::type_list_t<LeftTypes...>, jf::type_list_t<RightTypes...>> {
    using type = jf::type_list_t<LeftTypes..., RightTypes...>;
};
...
template <typename... LeftTypes, typename... RightTypes>
struct st_append_type<jf::type_list_t<LeftTypes...>, jf::type_list_t<RightTypes...>> {
    using type = jf::type_list_t<RightTypes..., LeftTypes...>;
};
}  // namespace hidden
...
template <typename... Types>
struct st_union_type<jf::type_list_t<Types...>> {
    using type = jf::type_list_t<Types...>;
};
template <typename... Types, typename Type>
struct st_union_type<jf::type_list_t<Types...>, Type> {
    using list = jf::type_list_t<Types...>;
    using type =
...
template <typename... Types, typename Type, typename... RightTypes>
struct st_union_type<jf::type_list_t<Types...>, Type, RightTypes...> {
    using list = st_union_type<jf::type_list_t<Types...>, Type>::type;
...
template <typename... Types>
struct st_union_type<jf::type_list_t<Types...>, jf::type_list_t<>> {
    using type = jf::type_list_t<Types...>;
};
template <typename... Types, typename Type>
struct st_union_type<jf::type_list_t<Types...>, jf::type_list_t<Type>> {
    using type = st_union_type<jf::type_list_t<Types...>, Type>::type;
};

template <typename... Types, typename Type, typename... RightTypes>
struct st_union_type<jf::type_list_t<Types...>, jf::type_list_t<Type, RightTypes...>> {
    using type = st_union_type<jf::type_list_t<Types...>, Type, RightTypes...>::type;
};
...
template <typename... Types>
struct st_intersection_type<jf::type_list_t<Types...>> {
    using type = jf::type_list_t<Types...>;
};

template <typename... Types, typename Type>
struct st_intersection_type<jf::type_list_t<Types...>, Type> {
    using list = jf::type_list_t<Types...>;
    using type = std::conditional_t<jf::is_type_in_list_v<Type, list>, jf::type_list_t<Type>,
                                    jf::type_list_t<>>;
};

template <typename... Types, typename Type, typename... RightTypes>
struct st_intersection_type<jf::type_list_t<Types...>, Type, RightTypes...> {
    using leftlist = jf::type_list_t<Types...>;
    using list = st_intersection_type<leftlist, Type>::type;

    using type = std::conditional_t<
        sizeof...(RightTypes) == 0, list,
        jf::prepend_type_t<list, typename st_intersection_type<leftlist, RightTypes...>::type>>;
};
/////////////////

template <typename... Types>
struct st_intersection_type<jf::type_list_t<Types...>, jf::type_list_t<>> {
    using type = jf::type_list_t<Types...>;
};

template <typename... Types, typename Type>
struct st_intersection_type<jf::type_list_t<Types...>, jf::type_list_t<Type>> {
    using type = st_intersection_type<jf::type_list_t<Types...>, Type>::type;
};

template <typename... Types, typename Type, typename... RightTypes>
struct st_intersection_type<jf::type_list_t<Types...>, jf::type_list_t<Type, RightTypes...>> {
    using type = st_intersection_type<jf::type_list_t<Types...>, Type, RightTypes...>::type;
};
...
struct st_remove_type<ArgType, Type> {
    using type =
        std::conditional_t<std::is_same_v<ArgType, Type>, jf::type_list_t<>, jf::type_list_t<Type>>;
};
...
template <typename ArgType>
struct st_remove_type<ArgType, jf::type_list_t<>> {
    using type = jf::type_list_t<>;
};

template <typename ArgType, typename Type>
struct st_remove_type<ArgType, jf::type_list_t<Type>> {
    using type = st_remove_type<ArgType, Type>::type;
};

template <typename ArgType, typename Type, typename... RightTypes>
struct st_remove_type<ArgType, jf::type_list_t<Type, RightTypes...>> {
...
template <typename... Types>
struct st_difference_type<jf::type_list_t<Types...>> {
    using type = jf::type_list_t<Types...>;
};

template <typename... Types, typename Type>
struct st_difference_type<jf::type_list_t<Types...>, Type> {
    using type = remove_type_t<Type, jf::type_list_t<Types...>>;
};

template <typename... Types, typename Type, typename... RightTypes>
struct st_difference_type<jf::type_list_t<Types...>, Type, RightTypes...> {
    using list = st_difference_type<jf::type_list_t<Types...>, Type>::type;
    using type = std::conditional_t<sizeof...(RightTypes) == 0, list,
...
template <typename... Types>
struct st_difference_type<jf::type_list_t<Types...>, jf::type_list_t<>> {
    using type = jf::type_list_t<Types...>;
};

template <typename... Types, typename Type>
struct st_difference_type<jf::type_list_t<Types...>, jf::type_list_t<Type>> {
    using type = st_difference_type<jf::type_list_t<Types...>, Type>::type;
};

template <typename... Types, typename Type, typename... RightTypes>
struct st_difference_type<jf::type_list_t<Types...>, jf::type_list_t<Type, RightTypes...>> {
    using type = st_difference_type<jf::type_list_t<Types...>, Type, RightTypes...>::type;
};
...
struct st_replace_type<NewType, OldType, Type> {
    using type = std::conditional_t<std::is_same_v<OldType, Type>, jf::type_list_t<NewType>,
                                    jf::type_list_t<Type>>;
};
...
template <typename NewType, typename OldType>
struct st_replace_type<NewType, OldType, jf::type_list_t<>> {
    using type = jf::type_list_t<>;
};

template <typename NewType, typename OldType, typename Type>
struct st_replace_type<NewType, OldType, jf::type_list_t<Type>> {
    using type = st_replace_type<NewType, OldType, Type>::type;
};

template <typename NewType, typename OldType, typename Type, typename... RightTypes>
struct st_replace_type<NewType, OldType, jf::type_list_t<Type, RightTypes...>> {
...
constexpr auto is_character_array_v =
    hidden::st_container_element<jf::remove_const_reference_t<T>>::is_character_array;

template <typename T>
constexpr auto array_count_v = hidden::st_container_element<jf::remove_const_reference_t<T>>::count;
...
using container_element_flat_t =
    hidden::st_container_element<jf::remove_const_reference_t<T>>::type;

template <typename T>
using container_element_t =
    hidden::st_container_element<jf::remove_const_reference_t<T>>::element_type;
...
auto make_vctr(Type&& first, Types&&... args) {
    if constexpr (std::is_array_v<jf::remove_const_reference_t<Type>>) {
        static_assert(is_character_array_v<Type>, "should be character array");

        using element_t = jf::container_element_t<Type>;
        using container_t = std::vector<element_t>;

        if (auto last_character = first[jf::array_count_v<Type> - 1];
            jf::container_element_flat_t<Type>{} == last_character)
            return container_t{jf::decay_array(first), jf::decay_array(args)...};
        else {
        ...
        using element_t = jf::remove_const_reference_t<Type>;
...
auto element_to_container(Type&& arg) {
    //array ..
    if constexpr (std::is_array_v<jf::remove_const_reference_t<Type>>) {
        if constexpr (jf::is_character_array_v<Type>) {
            using element_t = jf::container_element_t<Type>;

            if (const auto& last_character = arg[array_count_v<Type> - 1];
                jf::container_element_flat_t<Type>{} == last_character)
                return element_t{jf::decay_array(arg)};
            else
...
auto element_to_container(std::initializer_list<Type>& lst) {
    ...
    using element_t = jf::container_element_t<std::initializer_list<Type>>;
...
auto make_tuple(Type&& first, Types&&... args) {
    using container_t =
        std::tuple<jf::container_element_t<Type>, jf::container_element_t<Types>...>;
    return container_t{jf::element_to_container(std::forward<Type>(first)),
                       jf::element_to_container(std::forward<Types>(args))...};
}
...
constexpr auto are_all_same_flat_v =
    are_all_same_v<jf::remove_const_reference_t<T>, jf::remove_const_reference_t<Types>...>;
