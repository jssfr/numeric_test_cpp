...
template<types::number_c Number, class FuncType> constexpr auto bissec(Number interval1,
    Number interval2, FuncType&& func, double precision = 0.000005)
{
  if (func(interval1) * func(interval2) > 0) {
      throw std::invalid_argument(
          "can´t calculate because: func(interval1) * func(interval2) > 0. ");
  }

  double result = 0.0;
  for (int count {}; count < numConst::stop; ++count) {
      result = (interval1 + interval2) / 2;
      if (std::abs(func(result)) < precision) {
          return result;
      }

      if (numConst::signal(func(result)) == numConst::signal(func(interval1))) {
          interval1 = result;
      } else {
          interval2 = result;
        }
...
template<types::number_c Number, class FuncType> constexpr auto false_position(Number interval1,
    Number interval2, FuncType&& func, double precision = 0.000005)
{
...
template<types::number_c Number, class FuncType> constexpr auto newton_method(FuncType&& func,
    Number x0, double funcDelimiter = 0.000001, int order = 4)
{
  double result = 0.0;
  ...
  for (int count {}; count < numConst::stop; ++count) {
      ...
      trunc2 = std::trunc(result * std::pow(10, order)) / std::pow(10, order);
      if (trunc1 == trunc2 || std::abs(func(result)) < funcDelimiter) {
          return result;
      }
...
auto gauss_method(auto&& f1, auto&& f2, auto&& f3, types::tuple_or_array_c auto&& const_terms)
{
  ...
  auto [x0, y0, z0] = types::process_func_arg_pairs(f1, std::tuple { 1.f, 0.f, 0.f }, f1,
      std::tuple { 0.f, 1.f, 0.f }, f1, std::tuple { 0.f, 0.f, 1.f });

  auto [x1, y1, z1] = types::process_func_arg_pairs(f2, std::tuple { 1.f, 0.f, 0.f }, f2,
      std::tuple { 0.f, 1.f, 0.f }, f2, std::tuple { 0.f, 0.f, 1.f });

  auto [x2, y2, z2] = types::process_func_arg_pairs(f3, std::tuple { 1.f, 0.f, 0.f }, f3,
      std::tuple { 0.f, 1.f, 0.f }, f3, std::tuple { 0.f, 0.f, 1.f });
...
auto gauss_jacobi_method(auto&& f1, auto&& f2, auto&& f3,
    types::tuple_or_array_c auto&& const_terms, types::tuple_or_array_c auto&& initialStimative,
    int interations = 6)
{
  // terms multipling x, y and z
  auto [x0, y0, z0] = types::process_func_arg_pairs(f1, std::tuple { 1.f, 0.f, 0.f }, f1,
...
{
private:
  std::tuple<std::function<float(float, float, float)>, std::function<float(float, float, float)>,
      std::function<float(float, float, float)> >
      systems;
  std::tuple<float, float, float> rhds;

public:
  system3_solver(auto&& x0, auto&& y0, auto&& z0, types::number_c auto rhs1, auto&& x1, auto&& y1,
      auto&& z1, types::number_c auto rhs2, auto&& x2, auto&& y2, auto&& z2,
      types::number_c auto rhs3)
  ...
  auto solve_gauss_seidel(types::tuple_or_array_c auto&& initialStimative, int interations = 6)
      -> decltype(auto)
  {
    return gauss_seidel_method(std::get<0>(systems), std::get<1>(systems), std::get<2>(systems),
        rhds, initialStimative, interations);
  }
  auto solve_gauss_jacobi(types::tuple_or_array_c auto&& initialStimative, int interations = 6)
      -> decltype(auto)
  {
    return gauss_jacobi_method(std::get<0>(systems), std::get<1>(systems), std::get<2>(systems),
        rhds, initialStimative, interations);
  }
};
