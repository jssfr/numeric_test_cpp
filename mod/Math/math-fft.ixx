...
template<typename PolynomialType> auto get_even_odd(const PolynomialType& poly)
{
  ...
  for (std::size_t i {}; i < size; ++i) {
      if (i % 2 == 0) {
          even_poly.emplace_back(poly[i]);
      } else {
          odd_poly.emplace_back(poly[i]);
        }
    }
...
export template<typename PolynomialType> constexpr auto evaluate(const PolynomialType& poly)
{
  ...
  auto w = [v_2pi_n](auto j)
  { return std::exp(std::complex<element_t> { element_t {}, v_2pi_n * j }); };
...
template<typename PolynomialType> auto fft_interpolate_impl(const PolynomialType& poly)
{
...
export template<typename PolynomialType> auto interpolate(const PolynomialType& poly)
{
...
// fft derivative
template<std::size_t id, int Order, int N, typename FuncType, jf::types::number_c ArgType,
    jf::types::number_c... ArgTypes, auto count = jf::types::param_count_v<FuncType>>
auto fft_derivative_impl(FuncType&& func, ArgType arg, ArgTypes... args)
{
  ...
  auto ff = [&func, &w, argpack = std::tuple { arg, args... }](auto x)
  {
    ...
    return jf::types::lambda_seq<count>([&]<auto... i>(jf::types::sequence<i...>)
...
{
  return [func]<typename ArgType, typename... ArgTypes>(ArgType arg, ArgTypes... args)
  {
...
export template<std::size_t Order = 1, int N = 32, std::size_t id = 0, typename FuncType,
    typename... ParamTypes>
