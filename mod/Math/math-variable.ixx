...
template<auto N, auto n> auto create_oprvar() noexcept
{
  auto f = [](auto... args)
    requires(sizeof...(args) == N) { return std::get<n>(std::forward_as_tuple(args...)); };

  return 1 * f;
}

template<auto N, auto... n> auto variables() noexcept
{
  if constexpr (sizeof...(n) == 0) {
      return []<typename T, T... k>(std::integer_sequence<T, k...>)
      { return std::tuple { create_oprvar<N, k>()... }; }(std::make_integer_sequence<int, N> {});
  } else {
...
template<class F, class G> auto operator+(F&& f, G&& g) noexcept
{
  return [=]<typename T, typename... ts>(T x0, ts... args)
  { return (validate(f, x0, args...) + validate(g, x0, args...)); };
}

template<class F, jf::types::number_c Gtype> auto operator+(F&& f, Gtype&& g) noexcept
{
  return [=]<typename T, typename... ts>(T x0, ts... args)
  { return (validate(f, x0, args...) + g); };
}

template<jf::types::number_c Ftype, class G> auto operator+(Ftype&& f, G&& g) noexcept
{
  return [=]<typename T, typename... ts>(T x0, ts... args)
...
template<typename FuncType, typename NATType> auto operator,(FuncType&& f, NATType&& args) noexcept
{
