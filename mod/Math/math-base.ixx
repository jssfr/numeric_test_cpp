...
constexpr auto point_cart_cyl(types::number_c auto x, types::number_c auto y,
    types::number_c auto z)
{
...
constexpr auto point_cyl_cart(types::number_c auto rho, types::number_c auto phi,
    types::number_c auto z)
{
...
constexpr auto point_cart_spher(types::number_c auto x, types::number_c auto y,
    types::number_c auto z)
{
...
constexpr auto point_spher_cart(types::number_c auto r, types::number_c auto theta,
...
constexpr auto point_cyl_spher(types::number_c auto rho, types::number_c auto phi,
    types::number_c auto z)
{
...
constexpr auto point_spher_cyl(types::number_c auto r, types::number_c auto theta,
    types::number_c auto phi)
{
    ...
    types::number_c auto phi)
{
  ...
  var_cyl.set_value({ std::cos(phi), std::sin(phi), 0, -std::sin(phi), std::cos(phi), 0, 0, 0, 1 });

  jf::matrix::dmat coord_cart(3, 1);
  coord_cart.set_value(
...
auto cart_cyl(std::tuple<double, double, double>&& point, auto&&... funcs)
{
...
auto cyl_cart(std::tuple<double, double, double>&& point, auto&&... funcs)
{
  ...
  coord_cyl.set_value(
      { static_cast<double>(Arho), static_cast<double>(Aphi), static_cast<double>(Az) });
  jf::matrix::dmat coord_cart = var_cyl * coord_cyl;
...
auto cart_spher(double x, double y, double z, double theta, double phi)
{
  jf::matrix::dmat var_spher(3, 3);
  var_spher.set_value({ std::sin(theta) * std::cos(phi), std::sin(theta) * std::sin(phi),
      std::cos(theta), std::cos(theta) * std::cos(phi), std::cos(theta) * std::sin(phi),
      -std::sin(theta), -std::sin(phi), std::cos(phi), 0 });
...
auto spher_cart(double Ar, double Atheta, double Aphi, double theta, double phi)
{
  jf::matrix::dmat var_spher(3, 3);  // inverse
  var_spher.set_value({ std::sin(theta) * std::cos(phi), std::cos(theta) * std::cos(phi),
      -std::sin(phi), std::sin(theta) * std::sin(phi), std::cos(theta) * std::sin(phi),
      std::cos(phi), std::cos(theta), -std::sin(theta), 0 });
...
auto spher_cart(std::tuple<double, double, double>&& point, auto&&... funcs)
{
  ...
  coord_spher.set_value(
      { static_cast<double>(Ar), static_cast<double>(Atheta), static_cast<double>(Aphi) });
  jf::matrix::dmat coord_cart = var_spher * coord_spher;
...
auto spher_cyl(double Ar, double Atheta, double Aphi, double theta)
{
  jf::matrix::dmat var_spher(3, 3);  // inverse
  var_spher.set_value(
      { std::sin(theta), std::cos(theta), 0, 0, 0, 1, std::cos(theta), -std::sin(theta), 0 });
...
auto cyl_spher(double Arho, double Aphi, double z, double theta)
{
  jf::matrix::dmat var_cyl(3, 3);  // inverse
  var_cyl.set_value(
      { std::sin(theta), 0, std::cos(theta), std::cos(theta), 0, -std::sin(theta), 0, 1, 0 });
...
{
  auto [r, theta, phi] = point_cyl_spher(std::get<0>(point), std::get<1>(point),
      std::get<2>(point));
