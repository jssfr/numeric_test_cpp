...
auto cart_cyl(types::number_c auto&& Ax, types::number_c auto&& Ay, types::number_c auto&& Az,
    types::number_c auto phi)
{
  jf::matrix::dmat<3, 3> var_cyl;
  var_cyl.set_value({ { { std::cos(phi), std::sin(phi), 0 }, { -std::sin(phi), std::cos(phi), 0 },
      { 0, 0, 1 } } });

  jf::matrix::dmat<3, 1> coord_cart;
  coord_cart.set_value({ { { static_cast<double>(Ax) }, { static_cast<double>(Ay) },
...
auto cyl_cart(std::tuple<double, double, double>&& point, auto&&... funcs)
{
  ...
  coord_cyl.set_value({ { { static_cast<double>(Arho) }, { static_cast<double>(Aphi) },
      { static_cast<double>(Az) } } });
  jf::matrix::dmat coord_cart = var_cyl * coord_cyl;
...
{
  jf::matrix::dmat<3, 3> var_spher;  // inverse
  var_spher.set_value(
  ...
  jf::matrix::dmat<3, 1> coord_spher;
...
auto spher_cart(std::tuple<double, double, double>&& point, auto&&... funcs)
{
  ...
  coord_spher.set_value({ { { tatic_cast<double>(Ar) }, { static_cast<double>(Atheta) },
      { static_cast<double>(Aphi) } } });
  jf::matrix::dmat coord_cart = var_spher * coord_spher;
...
{
  jf::matrix::dmat<3, 3> var_spher;  // inverse
  var_spher.set_value({ { { std::sin(theta), std::cos(theta), 0 }, { 0, 0, 1 },
      { std::cos(theta), -std::sin(theta), 0 } } });
...
auto spher_cyl(std::tuple<double, double, double>&& point, auto&&... funcs)
{
  ...
  coord_spher.set_value({ { { static_cast<double>(Ar) }, { static_cast<double>(Atheta) },
      { static_cast<double>(Aphi) } } });
  jf::matrix::dmat coord_cyl = var_spher * coord_spher;
...
{
  jf::matrix::dmat<3, 3> var_cyl;  // inverse
