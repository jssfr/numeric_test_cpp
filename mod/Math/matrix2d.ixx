...
public:
  ...
  // unidimentional vector is faster then vector of vectors  aka
  // vector<vector<>>
private:
  ...
  template <typename IndexElementType,
            ...
            typename... Types>
  ElementType
  term(const IndexContainerType<IndexElementType, Types...> &index) const {
...
public:
  ...
  ElementType determinant_leibniz_parallel() const {
    ...
    parallel::parallel_for(std::size_t{}, static_cast<std::size_t>(max_permu),
                           inner_loop);
  ...
  ElementType determinant_laplace_parallel_atomic() const {
    if (this->m_rows == 0)
      ...
      auto handle =
          [&](auto &range) { // auto& ,aps to
            ...
            for (auto j = range.begin(); j < range.end(); ++j) {
              ...
              ElementType new_det =
                  (j % 2) ? old_det - cofactor : old_det + cofactor;
              while (!det.compare_exchange_strong(old_det, new_det)) {
                old_det = det;
      ...
      parallel::parallel_for(
          parallel::blocked_range{std::size_t{}, this->m_cols}, handle);
  ...
  ElementType determinant_laplace_parallel_reduce() const {
    if (this->m_rows == 0)
      ...
      auto sum_up = [](auto left_det, auto right_det) {
        return left_det + right_det;
      };

      return parallel::parallel_reduce(
          parallel::blocked_range{std::size_t{}, this->m_cols}, ElementType{},
          handle, sum_up);
    }
  }
  ...
  /// houseHolder method for orthogonal and upper triangular matrices for QR
  /// algorithm
  ...
  auto houseHolder_parallel() -> std::tuple<matrix_2d, matrix_2d> {
    ...
    auto sum_up = [](auto left_sum, auto right_sum) {
    ...
    for (std::size_t k = 0; k < cols; ++k) {
      ...
      parallel::parallel_for(
          parallel::blocked_range{k + 1, rows}, [&](auto &range) {
      ...
      for (std::size_t j = k; j < cols; ++j) {
        ...
        parallel::parallel_for(
            parallel::blocked_range{k, rows}, [&](auto &range) {
              for (std::size_t i = range.begin(); i < range.end(); ++i)
                R(i, j) -= 2 * v[i] * dot;
            });
      }
      ...
      // update Q
      parallel::parallel_for(
          parallel::blocked_range{std::size_t{}, rows}, [&](auto &range) {
            for (std::size_t i = range.begin(); i < range.end(); ++i) {
              value_type dot{};
              for (std::size_t j = k; j < cols; ++j) {
                dot += v[j] * Q(i, j);
  ...
  auto eigenvalues() -> std::vector<value_type> {
    ...
    if (this->m_rows == 1) {
      ...
      value_type d =
          std::sqrt(mean * mean - this->determinant_laplace_serial());
      eigs[0] = mean - d;
      eigs[1] = mean + d;
    } else {
      ...
      for (int n{}; n < 50; ++n) {
        matrix_2d shift =
            this->identity() * B(this->m_rows - 1, this->m_cols - 1);
        matrix_2d C = B - shift;
        B = C.multiRQ() + shift;
      }
  ...
  auto transpose() -> matrix_2d {
    ...
    if (this->m_rows > std::size_t{3} && this->m_cols > std::size_t{3}) {
      parallel::parallel_for(
          parallel::blocked_range{std::size_t{}, this->m_rows},
          [&](auto range) {
  ...
  auto eigenvectors() -> matrix_2d {
    ...
    auto solve_system = [&](value_type lambda) -> std::vector<value_type> {
      ...
      value_type euclideanNorm = std::sqrt(std::accumulate(
          eigenvector.begin(), eigenvector.end(), value_type{},
          [](value_type sum, value_type v) { return sum + v * v; }));
      std::ranges::for_each(
          eigenvector, [euclideanNorm](value_type &v) { v /= euclideanNorm; });
#else
      value_type euclideanNorm = std::sqrt(std::ranges::fold_left(
          eigenvector |
    ...
    if (num_eigenvalues > 3) {
      parallel::parallel_for(
          parallel::blocked_range{std::size_t{}, num_eigenvalues},
          [&](auto &range) {
...
using aligned_safe_matrix_2d_t =
    matrix_2d<ElementType, oneapi::tbb::concurrent_vector,
...
export using fmat =
    matrix_2d<float, std::vector, oneapi::tbb::scalable_allocator>;

export using dmat =
    matrix_2d<double, std::vector, oneapi::tbb::scalable_allocator>;

export using imat =
    matrix_2d<int, std::vector, oneapi::tbb::scalable_allocator>;
#else
...
struct fmt::formatter<
    jf::matrix::matrix_2d<ElementType, ContainerType, AllocatorType>> {
  constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(
      const jf::matrix::matrix_2d<ElementType, ContainerType, AllocatorType> &m,
