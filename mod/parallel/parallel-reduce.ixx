  ...
  for (unsigned i = 0; i < num_threads; ++i) {
      ...
      if (chunk_begin != chunk_end) {
          futures.push_back(std::async(std::launch::async,
              [=, &func]() { return std::ranges::fold_left(chunk_begin, chunk_end, T {}, func); }));
      }
    }
...
T parallel_reduce_impl(blocked_range range, T init, Func&& func, Proj&& proj)
{
  ...
  for (unsigned i = 0; i < num_threads; ++i) {
      ...
      std::size_t chunk_end = std::min(chunk_begin + chunk_size, range.end());

      if (chunk_begin != chunk_end) {
          blocked_range subrange { chunk_begin, chunk_end };
          futures.push_back(
              std::async(std::launch::async, [=, &func]() { return func(subrange, init); }));
      }
    }
