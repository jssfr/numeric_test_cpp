...
void parallel_for_impl(Index start, Index end, Index step, Func&& func)
{
  ...
  for (unsigned i = 0; i < num_threads; ++i) {
      Index chunk_start = start + i * chunk_size;
      Index chunk_end = std::min(chunk_start + chunk_size, end);
...
template<class Func> void parallel_for_impl(blocked_range range, Func&& func)
{
  const unsigned num_threads = std::thread::hardware_concurrency();
  ...
  for (unsigned i = 0; i < num_threads; ++i) {
      ...
      if (chunk_start < chunk_end) {
          ...
          threads.emplace_back([=, &func]() { func(subrange); });
      }
    }
}

export template<typename Index, class Func> void parallel_for(Index start, Index end, Func&& func)
{
...
export template<class Func> void parallel_for(blocked_range&& range, Func&& func)
{
