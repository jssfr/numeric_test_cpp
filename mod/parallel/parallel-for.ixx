...
namespace jf::par{
    ...
    template<typename Index, class Func>
    void parallel_for_impl(blocked_range range, Index step, Func&& func)
    {
        const unsigned num_threads = std::thread::hardware_concurrency();

        std::vector<std::jthread> threads;

        Index chunk_size = (range.end() - range.begin() + num_threads - 1) / num_threads;

        for (unsigned i = 0; i < num_threads; ++i) {
            Index chunk_start = range.begin() + i * chunk_size;
            Index chunk_end = std::min(chunk_start + chunk_size, range.end());
    ...
    {
        parallel_for_impl(range, static_cast<size_t>(1), func);
    }
    
    export template<typename Index, class Func>
    void parallel_for(blocked_range&& range, Index step, Func&& func)
    {
        parallel_for_impl(range, step, func);
    }
}